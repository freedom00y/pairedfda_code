...
for(type in c('t','n'))
{
  ...
  res_collect=matrix(as.double(res_collect),nrow=8,ncol=16,byrow=FALSE)
...
save(pt,pn,file = paste0(outpath,"Estimators.RData"))
```
...
pdf(paste0(outpath,"R_band_subjects_fitting.pdf"))
par(mfrow=c(5,5))
...
for(j in 1:n)
{
  ...
  y_cur = data$y[(ind_y[j]+1):ind_y[j+1]]
  ymin = min(y_cur)
  ymax = max(y_cur)
  yrange = ymax-ymin
  plot(time_y[(ind_y[j]+1):ind_y[j+1]],y_cur,
       cex=0.5,pch=21,main=paste(starlabel[j,1],starlabel[j,2],sep="-"),xlim=c(-10,50),ylim=c(ymin-0.1*yrange,ymax+0.1*yrange))
...
Fit I band for all subjects.

```{r echo=FALSE}
pdf(paste0(outpath,"I_band_subjects_fitting.pdf"))
par(mfrow=c(5,5))
par(mar=c(1.5,1.5,1.5,1.5))
for(j in 1:n)
{
  ...
  z_cur = data$z[(ind_z[j]+1):ind_z[j+1]]
  zmin = min(z_cur)
  zmax = max(z_cur)
  zrange = zmax-zmin
  plot(time_z[(ind_z[j]+1):ind_z[j+1]],z_cur,
       cex=0.5,pch=21,main=paste(starlabel[j,1],starlabel[j,2],sep="-"),xlim=c(-10,50), ylim=c(zmin-0.1*zrange,zmax+0.1*zrange))
...
## Use ggplot to plot all figures
```{r plot mean curves, echo=FALSE}
library(ggplot2)
x = seq(-10,50,0.1)
Bval = orthbasis(x,bknots)$B
...
```{r Fitting error, echo=FALSE}
#source('Total_Square_Error.R')
t_mse = TSE(data,pt,data$nobs_y,data$nobs_z)
#s_mse = TSE(data,ps,data$nobs_y,data$nobs_z)
n_mse = TSE(data,pn,data$nobs_y,data$nobs_z)
...
titles = c("SN2006py", "SN2008cm","SN2009al", "SN2009gf", "SN2010ai", "SN2010Y")
title_ind=c()
for(i in 1:length(titles))
{
  title_ind = c(title_ind,which(filenames==paste0(titles[i],".txt")))
...
sub = rep(titles,each = 4*length(x)) 
sub = factor(sub,levels=titles)
Band = factor( rep( rep(c("R","I"),each=2*length(x)), length(titles)) ,levels=c("R","I") )
model = rep( rep(c("RRME-t","RRME-normal","RRME-t","RRME-normal"), each = length(x)), length(titles)) 
...
pt_sub=factor(pt_sub,levels=titles)
pt_band=factor(pt_band, levels = c("R","I") )
df_sub_pt = data.frame(pt_x,pt_y,Band = pt_band, sub=pt_sub)
...
p_sub = ggplot(df_sub) + 
  geom_line(aes(x=Phase, y=Mag, linetype=model,color=model), size=1) + 
  scale_color_manual(values=c("red", "blue")) +
  facet_grid(vars(Band), vars(sub)) + ylim(3.4,-0.2) +
  geom_point(data = df_sub_pt, aes(x=pt_x, y=pt_y), size=1) +
  facet_grid(vars(Band), vars(sub),
             labeller = labeller(Band = band.labs)) + ylab("Magnitude") + 
  theme(legend.title = element_text(size = 10), 
               legend.text = element_text(size = 10))
ggsave("unlabelled.pdf",p_sub, width=14, height=5, units="in")
```
