...
percent = 0.01
freedom_range=c(1)
filename=paste('simu_output_outlier/curveoutlier_',varres,'_',simu_times,'_',percent,'.txt', sep = "")
title=paste('Generate from',type, 'distribution.',
            'Repeat',simu_times,'times.',
            'The variance of residual is',varres,'.',
            'The outlier observations is percent =',percent,'.')
...
for(free in freedom_range)
{
  ...
  clusterExport(cl,c("free","filename","varres","type","percent"))
  res1=parSapply(cl, 
                 1:simu_times+7, 
                 function(i){
                   source('onesimu_curveoutlier.R')
                   temp=try(simu(seed = i,type,free,varres,percent))
