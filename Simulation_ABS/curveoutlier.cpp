...
//[[Rcpp::export]]
List curveoutlier(const int& n, const double& varres, const double& percent=0.01, const int ka=2, const int kb=2){
  ...
  for(int i=0;i<n;i++)
  {
    ...
    vec eps = mvnrnd( zeros(nknot(i)), eps0*eye(nknot(i),nknot(i)), 1);
    vec xi  = mvnrnd( zeros(nknot(i)), xi0*eye(nknot(i),nknot(i)), 1);
